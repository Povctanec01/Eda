...
def register_fonts():
    ...
    try:
        ...
        if os.path.exists(font_path):
            ...
            # Если нет Arial, используем DejaVu Sans
            dejavu_paths = [
    ...
    if request.method == 'POST':
        ...
        for username in usernames:
            try:
                ...
                #Запрет на удаление себя и админов
...
#Статистика заказов
@login_required
def admin_statistics_buys(request):
    if not request.user.is_authenticated or request.user.profile.role != 'admin':
        return redirect('login')
    ...
    return render(request, 'main/admin_dashboard/admin_statistics_buys.html', {
